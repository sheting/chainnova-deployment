...
echo -e "$BASE_BUILD_REPOSITORY:$BASE_TAG\n$BASE_DEV_REPOSITORY:$BASE_TAG\n$BASE_SERVICE_REPOSITORY:$SERVICE_BASE_TAG\n">>.tmp_base_image
...
echo "########################################"
docker run --rm --env-file $(pwd)/$SRC_PATH/.env $BUILD_ENV -v $(pwd)/$SRC_PATH:/code -v $(pwd)/../../deploy_fe_wp:/code/deploy_fe_wp -w /code ${BASE_BUILD_REPOSITORY}:${BASE_TAG} ./deploy_fe_wp/src/build_wp.sh
...
echo "##### BUILD IMAGE "
echo "########################################"
docker build --no-cache --build-arg BASE_REPOSITORY=${BASE_SERVICE_REPOSITORY} --build-arg BASE_TAG=${SERVICE_BASE_TAG} --build-arg MODE_TYPE=$__MODE_TYPE -f ../../deploy_fe_wp/Dockerfile_service -t $IMAGE_LOCAL ./$SRC_PATH
if [ $? != 0 ]; then
